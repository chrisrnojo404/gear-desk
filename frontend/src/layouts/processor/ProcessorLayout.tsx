...
const navigationItems = [
  { to: '/dashboard/processor', label: 'Dashboard', end: true },
  { to: '/dashboard/processor/all-requests', label: 'Alle aanvragen' },
  { to: '/dashboard/processor/pending', label: 'Pending aanvragen' },
  { to: '/dashboard/processor/approved', label: 'Goedgekeurde aanvragen' },
  { to: '/dashboard/processor/in-use', label: 'In gebruik aanvragen' },
]
      ...
      <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-7xl gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <main className="panel-glow rounded-[2rem] border border-white/10 bg-panel-900/85 p-5 backdrop-blur-xl sm:p-6 lg:p-8">
          <div className="mb-6 flex flex-col gap-3 rounded-[1.75rem] border border-white/10 bg-white/5 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs tracking-[0.18em] text-slate-400 uppercase">Level 2 omgeving</p>
              <p className="mt-1 text-sm text-slate-300">
                Verwerk aanvragen, keur sets goed en houd apparatuur in gebruik bij.
              </p>
            </div>
            <div className="rounded-full bg-sand-50 px-4 py-2 text-sm font-medium text-slate-900">
              Ingelogd als verwerker
            </div>
          </div>
